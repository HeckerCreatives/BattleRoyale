        ...
        public virtual void DrawRegisteredPrefabsBoxList()
        {
            if ((Application.isPlaying || _manager.isInitialized) && _manager.runtimeDataList != null && _manager.runtimeDataList.Count > 0)
            {
                ...
                foreach (GPUInstancerRuntimeData runtimeData in _manager.runtimeDataList)
                {
                    ...
                    totalInstanceCount += runtimeData.instanceCount;
                    totalDrawCallCount += drawCallCount;
                    totalShadowDrawCall += shadowDrawCallCount;
                    if (runtimeData.instanceCount > 0 && runtimeData.instanceCount < 100)
                        hasLowInstanceCounts = true;
                }

                GUILayout.Label("\nTotal Instance Count: " + String.Format("{0:n0}", totalInstanceCount) +
                ...
                if (Application.isPlaying && !GPUInstancerConstants.gpuiSettings.disableInstanceCountWarning && hasLowInstanceCounts && _manager is GPUInstancerPrefabManager)
                {
                    GUILayout.Space(5);

...
public class PlayerUpperFallingRange : UpperNoAimState
{
    public PlayerUpperFallingRange(SimpleKCC characterController, UpperBodyChanger playablesChanger, PlayerMovementV2 playerMovement, PlayerPlayables playerPlayables, AnimationMixerPlayable mixerAnimations, List<string> animations, List<string> mixers, string animationname, string mixername, float animationLength, AnimationClipPlayable animationClipPlayable, bool oncePlay) : base(characterController, playablesChanger, playerMovement, playerPlayables, mixerAnimations, animations, mixers, animationname, mixername, animationLength, animationClipPlayable, oncePlay)
    {
    }
    ...
    private void WeaponsChecker()
    {
        if (playerMovement.Attacking)
        {
            ...
            {
                if (playerPlayables.inventory.SecondaryWeaponID() == "003" && playerPlayables.inventory.SecondaryWeapon.Supplies > 0)
                    playablesChanger.ChangeState(playerPlayables.upperBodyMovement.RifleShootPlayable);

                else if (playerPlayables.inventory.SecondaryWeaponID() == "004" && (playerPlayables.inventory.SecondaryWeapon.Supplies > 0 || playerPlayables.inventory.BowMagazine > 0))
        ...
        {
            if (playerPlayables.inventory.WeaponIndex == 1 || playerPlayables.inventory.WeaponIndex == 2)
            {
